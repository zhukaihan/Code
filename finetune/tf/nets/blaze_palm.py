...
def blaze_palm_block(x, filters, strides):
    ...
    if strides == 2:
        ...
        max_pooling = tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=strides, padding='same')(x)
        if input_channel != output_channel:
            paddings = tf.constant([[0, 0], [0, 0], [0, 0], [0, max_pooling.shape[-1]]], dtype=tf.int32)
            channel_pad = tf.pad(max_pooling, paddings)
            x = channel_pad
        else:
            x = max_pooling
...
def build_blaze_palm_model(to_64=False):
    ...
    #     cls_64 = tf.keras.layers.Conv2D(filters=6, kernel_size=(1, 1), strides=(1, 1), padding='same')(blaze_palm_block_8)
    #     reshape_cls_8 = tf.keras.layers.Reshape([-1, 1])(cls_8)
    #     reshape_cls_16 = tf.keras.layers.Reshape([-1, 1])(cls_16)
    #     reshape_cls_32 = tf.keras.layers.Reshape([-1, 1])(cls_32)
    #     reshape_cls_64 = tf.keras.layers.Reshape([-1, 1])(cls_64)
    #     loc = tf.keras.layers.Concatenate(axis=1, name='regressors')([reshape_reg_8, reshape_reg_16, reshape_reg_32, reshape_reg_64])
    #     conf = tf.keras.layers.Concatenate(axis=1, name='classfiers')([reshape_cls_8, reshape_cls_16, reshape_cls_32, reshape_cls_64])

    # x_out = {
    #     'regressors': loc, 
    #     'classifiers': conf
    #     }#tf.concat([conf, loc], axis=-1)
